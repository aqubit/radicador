 ...
 * roles.java
 *
 * Created on 1 de marzo de 2006, 09:18 AM
 * Copyright César
 */
 ...
 */
public class roles extends AbstractPageBean {
    ...
    public void init() {
        ...
        try {
            ...
            log("roles Initialization Failure", e);
        ...
        tablaRolDataProvider = getAdminSessionBean().getTablaRolDataProvider();
        rol  = getAdminSessionBean().getRolEnEdicion();
        //Poblar los componentes con datos
        tablaRolDataProvider.setList( svc.getRoles() );
    }
    /**
    ...
    public void setRol(RolVo rol) {
        this.rol = rol;
        getAdminSessionBean().setRolEnEdicion( rol );
    }
    private SipDataProvider tablaRolDataProvider;
    ...
    public String btnEditar_action() {
        ...
        if( selectedRowKeys.length > 0 ){
            RolVo per = (RolVo)tablaRolDataProvider.getObject(selectedRowKeys[0]);
            setRol(per);
    ...
    public String btnBorrar_action() {
       ...
       RowKey[] selectedRowKeys = getTableRowGroup1().getSelectedRowKeys();
       if( selectedRowKeys.length > 0 ){
           RolVo ob = (RolVo)tablaRolDataProvider.getObject(selectedRowKeys[0]);
           ob.setIdUsuario(getAdminSessionBean().getUsuarioLogged().getId_usuario());
           if( !svc.deleteRol(ob)){
               error(guiMessages.getString("errorRolYaRelacionado"));
           }else{
               resetComponentes();
               form1.discardSubmittedValues("agregar");
           }
