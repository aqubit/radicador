 ...
 * download.java
 *
 * Created on 23 de mayo de 2006, 05:52 PM
 * Copyright César
 */
 ...
 */
public class download extends AbstractPageBean {
    ...
    public void init() {
        ...
        try {
            ...
            log("download Initialization Failure", e);
        ...
        // TODO - agregue aquí su propio código de inicialización
        CorrespondenciaVo corresp = getCorrespondenciaSessionBean().getCorrespondenciaActual();
        String downloadFile = corresp.getDocumento().getNombre_documento();
        ServletContext context = (ServletContext) getExternalContext().getContext();
        HttpServletResponse response = (HttpServletResponse) getExternalContext().getResponse();
        String strContentType = null;
        if( downloadFile.endsWith("doc")){
            strContentType = "application/msword";
        }else if(downloadFile.endsWith("pdf")){
            strContentType = "application/pdf";
        }else{
            strContentType = "image/jpeg";
        }
        byte[] byArrArchivo = corresp.getDocumento().getArchivo();
        response.setContentType(strContentType);
        response.addHeader("Content-Disposition", "attachment; filename=\"" + downloadFile + "\"");
        try{
            ServletOutputStream out = response.getOutputStream();
            response.setContentLength( byArrArchivo.length);
            out.write(byArrArchivo);
            out.close();
        }catch (Exception exc){
          exc.printStackTrace();
        }
